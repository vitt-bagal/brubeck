...
static int memcmpct(const void *_a, const void *_b, size_t len) {
...
static const char *hmactos(const char *buffer) {
...
static int verify_token(struct brubeck_server *server,
                        struct brubeck_statsd_secure *statsd,
                        const char *buffer) {
  uint32_t ha, hb;
  uint64_t timestamp;
  struct timespec now;
  ...
  if (statsd->now < timestamp) {
    log_splunk(
        ...
        (long long unsigned int)statsd->now, (long long unsigned int)timestamp);
  ...
  if (statsd->now - timestamp > statsd->drift) {
    log_splunk("sampler=statsd-secure event=fail_delayed now=%llu "
               "timestamp=%llu drift=%d",
  ...
  if (multibloom_check(statsd->replays, timestamp % statsd->drift, ha, hb)) {
    log_splunk("sampler=statsd-secure event=fail_replayed hmac=%s",
               hmactos(buffer));
...
static void *statsd_secure__thread(void *_in) {
  ...
  HMAC_Init_ex(ctx, statsd->hmac_key, strlen(statsd->hmac_key), SHA_FUNCTION(),
               NULL);

  for (;;) {
    int res = recvfrom(statsd->sampler.in_sock, buffer, sizeof(buffer) - 1, 0,
    ...
    if (memcmpct(buffer, hmac_buffer, SHA_SIZE) != 0) {
      log_splunk("sampler=statsd-secure event=fail_auth hmac=%s",
                 hmactos(buffer));
      brubeck_stats_inc(server, secure.failed);
      continue;
    }
...
static void shutdown_sampler(struct brubeck_sampler *sampler) {
  struct brubeck_statsd_secure *statsd =
      (struct brubeck_statsd_secure *)sampler;
  pthread_cancel(statsd->thread);
}

struct brubeck_sampler *brubeck_statsd_secure_new(struct brubeck_server *server,
                                                  json_t *settings) {
  struct brubeck_statsd_secure *std =
      xmalloc(sizeof(struct brubeck_statsd_secure));
  char *address;
  ...
  json_unpack_or_die(settings, "{s:s, s:i, s:s, s:i, s:i}", "address", &address,
                     "port", &port, "hmac_key", &std->hmac_key, "max_drift",
                     &drift, "replay_len", &replay_len);

  brubeck_sampler_init_inet((struct brubeck_sampler *)std, server, address,
                            port);
